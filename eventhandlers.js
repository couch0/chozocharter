...
function setHandlers(bot, socket, config) {
  ...
  if (socket.io._callbacks.length > 4) return false;
  ...
  socket.on("chatMsg", function(data) {
    ...
    if (!isMuted && !bot.userIsBot(data.username) && !cleanMsg.startsWith("🤖")) {
      ...
      if (!me
        ...
        try {
          (bot.cfg.interface.logChat ? bot.logger.verbose : bot.logger.verbose_nostream)(strings.format(bot, "COMMAND_ATTEMPT", [
    ...
    if (isServer) {
      ...
      if (me) {
        (bot.cfg.interface.logChat ? bot.logger.log : bot.logger.log_nostream)(C.blackBright("<") + username + " " + C.blackBright(outputMsg + "> "));
      } else {
        let endBracket = "> ";
        if (isMuted && bot.cfg.interface.indicateMutedUsers) endBracket = " (MUTED)" + endBracket;
        (bot.cfg.interface.logChat ? bot.logger.log : bot.logger.log_nostream)(C.blackBright("<") + username + C.blackBright(endBracket) + outputMsg);
      }
    }
	
	//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	if (!bot.userIsBot(data.username)){
		let ui = bot.creditUserIndex(data.username);
		if (ui)
		{
			if (outputMsg !== bot.credit[ui][3])
			{
				bot.credit[ui][3] = outputMsg;
				bot.addCredit(data.username, 1);
			}else{
				if (bot.spam){bot.addCredit(data.username, 1);}
			}
		}
	}
	//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  socket.on("mediaUpdate", function(data) {
	...
	let usr = pl[0].queueby;
	let tit = pl[0].media.title;
	let now = new Date;
	...
	tit = escapeQuotes(tit);
	bot.vidHistory[0] = [usr +"  " + "[" + ("0" + now.getUTCHours()).slice(-2) + ":" + ("0" + now.getUTCMinutes()).slice(-2) + ":" + ("0" + now.getUTCSeconds()).slice(-2) + "]", tit];
	if (!bot.minutely){bot.updateAll();}
	//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  });
  ...
  */
  ...
  socket.on("pm", function(data) {
    ...
    if (data.to === bot.username) {
      ...
      if (!sender) return;
      (bot.cfg.interface.logChat ? bot.logger.info : bot.logger.info_nostream)(strings.format(bot, "PM_RECV", [
          ...
          if (bot.cfg.moderation.notifyNewUser) {
            ...
            if (j >= end-1 && newUsers.length > 0) {
              if (newUsers.length > 5)
                bot.broadcastModPM(strings.format(bot, "NEW_USERS", [newUsers.slice(0,5).join(", ") + " (" + newUsers.length - 5 + " more)"]));
              else
