...
var clicommands = {
  ...
  "testdb": function(bot, cmd, message) {
    bot.db.run("testQuery", null, function(res) {
      if (res && res.rowCount >= 1) {
        bot.logger.info("Database connection OK!");
      } else {
        bot.logger.error("Database connection test query failed.");
      }
    })
  },
  ...
  "trigger": function(bot, cmd, message) {
    if (message.length >= 1) {
      bot.trigger = bot.validateTrigger(message[0]);
      bot.logger.info("Changed trigger to " + bot.trigger);
    }
  },
	//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
	"userlist": function(bot, cmd, message) {
		for (let i = 0; i < bot.creditRecord.length; i++) {
			bot.logger.log(i + ") " + C.red(bot.creditRecord[i][0]) + " " + C.yellowBright(bot.creditRecord[i][1]));
		}
	},
	"tspam": function(bot, cmd, message) {
		if (bot.spam){bot.spam = false; console.log("not including spam");}else{bot.spam = true; console.log("including spam");}
	},
	"tminutely": function(bot, cmd, message) {
		if (bot.minutely){bot.minutely = false; console.log("checking every 5 secs");}else{bot.minutely = true; console.log("checking every minute");}
	}
	//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
}
